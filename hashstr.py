    ...
    def run(self, edit, hashName='md5'):
        ...
        if view.sel()[0].empty() and len(view.sel()) == 1:  # No selection
            ...
            if(hashName.startswith('enc_base') or hashName.startswith('dec_base')):
                result = base(
                    arr2str([x for x in view.substr(sublime.Region(0, self.view.size())).splitlines(True) if x != '']), hashName)
            else:
                result = genHash(
                    arr2str([x for x in view.substr(sublime.Region(0, self.view.size())).splitlines(True) if x != '']), hashName)
            ...
            for region in view.sel():
                if region.empty():
                    ...
                    if(hashName.startswith('enc_base') or hashName.startswith('dec_base')):
                        result = base(
                            arr2str([x for x in view.substr(region).splitlines(True) if x != '']), hashName)
                    else:
                        result = genHash(
                            arr2str([x for x in view.substr(region).splitlines(True) if x != '']), hashName)
